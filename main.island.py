...
BOT_TOKEN = "8885304264:AAGX9o2KKtWMwnEYduPCiBFtz-f8ldYC7mw"
ADMIN_IDS = {
    int(x.strip())
    for x in "8872934046".split(",")
...
async def cb_island(callback: CallbackQuery):
    ...
    # Если игрок уже находится на экране острова, Telegram не разрешает
    # повторно редактировать сообщение без изменений.
    text = home_text(user.id)
    if callback.message.text == text:
        await callback.answer("🏝️ Вы уже на своём острове!")
        return

    await callback.answer("🏝️ Остров открыт!")
...
async def cb_gather(callback: CallbackQuery):
    ...
    text = (
        ...
        f"⛏ Камень: {cooldown_text(user_id, 'stone') or 'готово'}\n"
...
async def cb_gather_action(callback: CallbackQuery):
    ...
    if action == "mine":
        ...
        set_cd(user_id, "stone", COOLDOWNS["stone"])
